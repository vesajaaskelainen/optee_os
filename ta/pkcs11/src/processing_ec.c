 ...
 *   ta/pkcs11/scripts/dump_ec_params.sh
 */
				       ...
				       size_t tee_size)
{
	...
	if (qsize < 0x80) {
		...
		EMSG("DER long definitive form size > 255 not yet supported");
			  ...
			  struct obj_attrs **priv_head)
{
	...
	TEE_InitValueAttribute(&tee_key_attr[0], TEE_ATTR_ECC_CURVE, tee_curve,
			       0);
